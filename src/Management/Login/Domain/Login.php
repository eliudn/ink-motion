...
use Src\Management\Login\Domain\Exceptions\NoLoginException;
...
final class Login extends  Domain
{
    ...
    public function handler():array
    {
        ...
        return [
            'id'=>$this->entity()["id"],
            'nickname'=>$this->entity()["nick_name"],
    ...
    protected function isException(?string $exception): void
    {
        ...
        if (!is_null($exception)){
            match ($exception){
                self::USER_OR_PASSWORD_INCORRECT => throw new NoLoginException('email or password incorrect', $this->badRequest())
            };
        }
    }
